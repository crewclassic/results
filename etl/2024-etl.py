...
if __name__ == "__main__":
    ...
    file = open("2024.csv", mode="w", newline="")
    ...
    for entry in fetch_race_entries_line_by_line(resultsurl, entrySelect):
        match = re.search(r"Final", entry['raceLabel'])
        if match:
...
#                    print(f"{seat}: {crewIndex[EntryPID][regClubId][seat]}")
                ...
                output.writerow(["2024",entry['eventId'],entry['trophy'],entry['eventDescription'],entry['raceLabel'],entry['finishPlace'],entry['ClubName'],entry['adjTime']] + crew + [total_seconds])
